    ...
    public LLMResponse generate(Conversation on) throws HttpException, JsonProcessingException {
        ...
        try{
            ...
            if (null != terminalMessage) {
                return LLMResponse.builder().role("system").content(message).terminalMessage(terminalMessage.asText()).alert(alert).build();
            }
            return LLMResponse.builder().role("system").content(message).alert(alert).build();
        } catch (Exception e) {
            log.info("Error parsing compliance score: " + hello.concatenateResponses());
            return LLMResponse.builder().role("system").content("Error parsing response").build();
        }
    }
