...
public class ConnectedSystem {
    ...
    public TerminalLogDTO toDTO(String encryptedId) {
        var builder = TerminalLogDTO.builder();
        builder.sessionId(encryptedId);
        builder.user(this.user.getUsername());
        builder.host(this.hostSystem.getHost());
        builder.closed(this.session.getClosed());
        builder.sessionTime(this.session.getSessionTm());
        return builder.build();
    }

    public HostSystemDTO toHostSystemDTO(String hostConnectionString) {
        var dto = getHostSystem().toDTO();
        dto.setHostConnection(hostConnectionString);
        dto.setLastAccessed(TimeAgoFormatter.formatTimestampToMinutesAgo(getSession().getSessionTm()));
        return dto;
    }
}
