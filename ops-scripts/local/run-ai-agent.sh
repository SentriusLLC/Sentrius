...
mvn spring-boot:run \
  -Dspring-boot.run.jvmArguments="-Xms${MIN_HEAP} -Xmx${MAX_HEAP}"   -Dspring-boot.run.arguments="--spring.config.location=file:./src/main/resources/assessor.properties --agent.ai.config=assessor-config.yaml"
